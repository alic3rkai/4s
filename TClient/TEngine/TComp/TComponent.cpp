...
HRESULT TComponent::DrawText()
{
	...
	if( m_strText != m_strPrvText )
	{
		...
		m_bDidShrink = FALSE; // �ؿ� UpdateTextOutputData���� m_bDidShrink�� TRUE�� �ٽ� �����Ѵ�.
	}
...
void TComponent::ShrinkText( CString& text)
{
	...
	m_bDidShrink = TRUE; // ���ڰ� ©���� ���콺Ŀ���� �÷������� Ǯ�ؽ�Ʈ�� ������ ����� Ȱ��ȭ��Ŵ
}
			...
			if( szSIZE.cx > szCTRL.cx )
			{
				...
				while(!strLEFT.IsEmpty())
				{
					...
					else
					{
						...
						if( sizeWORD.cx > szCTRL.cx ) // �� �ܾ� ���̰� ������Ʈ�� ���α��̺��� ���. �̷��� ������ �ڸ���.
						{
							CSize sizeLINE;
							...
							// strNewWord�� strLINE�� ����, ������ strLEFT�� �ǵ��ư�.
	...
	else
	{
		...
		while(!strTEXT.IsEmpty())
		{
			...
			while(!strTEXT.IsEmpty())
			{
				...
				while( szTEXT.cx > szCTRL.cx ) // ���ڸ� �Ѱ��� ���鼭 szCTRL.cy �ȿ� �� ������ �ڸ���.
				{
...
VOID TComponent::UpdateTextOutputData( VECTORSTRING& vTextLine, const CSize szCTRL, TextOutputDataSet& outRESULT )
{
	if( !vTextLine.empty() )
	{
		...
		int nLINE = INT(min( szCTRL.cy, szTEXT.cy) * 1.2f);			// �� ���� ����(�ȼ�)
		int nTEXT = INT(vTextLine.size());							// ����ؾߵ� ��ü ���μ�
		nLINE = min( szCTRL.cy, nLINE);

		int nSLOT = min(szCTRL.cy/nLINE, nTEXT);					// ����� �� �ִ� ���� ���μ�
